...
Karen Cranston$^{(affil?)}$, Manish Parashar$^{(4)}$, David Proctor$^{(affil?)}$, \\
Matthew Turk$^{(2)}$, Colin Venters$^{(affil?)}$, Nancy Wilkins-Diehr$^{(5)}$}
...
\item encouraging industry's role in the sustainability of scientific software
\begin{itemize}
...
\todo{discuss grouping - well-sorted... and breakouts at WSSSPE2}



\section{Results}

The contributed papers that will be discussed at the workshop follow, listed by \todo{update when possible - current list is just alphabetic order, not grouped}
...
\item ed Habermann, Andrew Collette, Steve Vincena, Werner Benger, Jay Jay Billings, Matt Gerring, Konrad Hinsen, Pierre de Buyl, Mark K\"{o}nnecke, Filipe Rnc Maia and Suren Byna. The Hierarchical Data Format ({HDF}): A Foundation for Sustainable Data and Software~\cite{wssspe2_habermann}
