...
creation, and in Science, taking advantage of the open web. 

Thaney started noting the unfortunate fact that many current systems have been designed to create friction hindering us, despite the original intentions.%---social issues hindering us
An example is the National Cancer Institute's caBIG. 
A total of $\$350m$ was spent, more than $\$60m$ for management, $70+$ tools created, but still seen
...
\note{writing in progress}
Discussion Extracts

Who to incentivize: 
Those in science (to do better, more shareable, more sustainable work).
Those in industry (but are interested in science).

Giving credit in the traditional way:  software papers (JORS?).
Difficulty of adding new authors to publications
 - original authors disproportionately benefit
 - how can you add authors?
Can we control the end-users?
 - Yes, but we prefer something not so "confrontational"
Versioning.  How can we add that to publications? To add on people.  JSON-LD percentages.
John Towns: What do you want to encourage?
 - "good computational work" to advance their careers.
 - People to realize that I'm doing good software.
 - Empirical Q: Most of the developers are not going into traditional scientific careers.
    - Why are people going to Industry?  Money? Or working without worrying about careers paths
Portable reputation.
Trying to support career paths of "those in the grey area (between science and software - in development of software)"
 - How do you ensure that those "engineers" do not block the path "back into science".  Force the PIs to make programmers co-authors.

Prizes: reframe software as "instrument building"
	will prizes be good or bad, how can we
Converting reputation by joining apache software foundation.
	Google etc.  Inviting ASF and open source people to contribute to scientific code (at least on questions of overlapping

Template for assessing scientific contributions made through software.


making decisions about who to fund from funder perspective - who did what in the project?

someone writes paper in some area, adds references at end to make sure it’s clear that the paper fits in a field, but maybe don't think about how we use citations to give credit - how to rank/say what citations really matter, different from those that just explain the field (required background)
...
\note{writing to be done}

\url{http://tinyurl.com/l4tc6a4}
...
\subsubsection{Remaining breakout discussions: Prizes}
\note{writing to be done}
